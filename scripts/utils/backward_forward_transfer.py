...
def main():
    ...
    runid = args.runid
    file = args.file
    ckpt_path = f'trained_models/{runid}/{file}'
    ...
    iteration=checkpoint['global_step']//10
    if iteration % 100 != 0:
    ...
    print("Logging forward/backward transfer metric...")

    wandb.init(
        project="neurips-fbwt",
        entity="[author1]",
        name=f"fbwt-{cfg.method}-{dataset_name}-{runid}-{iteration}",
        config=raw_cfg
    )
    ...
    N = len(dataset)
    resolution = 6
    idxs = torch.linspace(0, N, N//(resolution)+1).long()

    for i, (from_idx, to_idx) in enumerate(zip(idxs[:-1], idxs[1:])):
        accuracy_at_time = matches[from_idx:to_idx].float().mean()
        wandb.log({
