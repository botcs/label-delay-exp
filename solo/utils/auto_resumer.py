...
Checkpoint = namedtuple("Checkpoint", ["creation_time", "args", "checkpoint","iter"])
...
class AutoResumer:
    SHOULD_MATCH = [
        ...
        "online.delay",
        "method_kwargs",
        "online.sup_buffer_size",
        "online.imbalanced.enabled",
        "online.imbalanced.supervised",
        "online.imbalanced.unsupervised",
        "optimizer.name",
                ...
                for checkpoint_file in checkpoint_files:
                    ...
                    if current_time - creation_time < self.max_hours:
                        ck = Checkpoint(
                            ...
                            iter=int(name.strip('.ckpt').split('-')[-1][2:])
                        )
                        candidates.append(ck)

        if candidates:
            # sort by most recent
            candidates = sorted(candidates, key=lambda ck: ck.iter, reverse=True)
