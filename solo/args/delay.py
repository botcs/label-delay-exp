...
def add_and_assert_dataset_cfg(cfg: omegaconf.DictConfig) -> omegaconf.DictConfig:
    ...
    cfg.data.prefetch_factor = omegaconf_select(cfg, "data.prefetch_factor", 10)
...
def add_and_assert_wandb_cfg(cfg: omegaconf.DictConfig) -> omegaconf.DictConfig:
    ...
    cfg.log_every_n_steps = omegaconf_select(cfg, "log_every_n_steps", 20)
    cfg.log_histogram_every_n_steps = omegaconf_select(cfg, "log_histogram_every_n_steps", 100)
    return cfg
...
def add_and_assert_lightning_cfg(cfg: omegaconf.DictConfig) -> omegaconf.DictConfig:
    ...
    cfg.ckpt_path = omegaconf_select(cfg, "ckpt_path", None)
    cfg.checkpoint.train_time_interval = omegaconf_select(cfg, "checkpoint.train_time_interval", 3500)

    return cfg


def add_and_assert_online_cfg(cfg: omegaconf.DictConfig) -> omegaconf.DictConfig:
    """Adds specific default values/checks for online config.
    ...
    cfg.online = omegaconf_select(cfg, "online", {})
    cfg.online.delay = omegaconf_select(cfg, "online.delay", 0)
    cfg.online.sup_buffer_size = omegaconf_select(cfg, "online.sup_buffer_size", 40000)
    cfg.online.num_supervised = omegaconf_select(cfg, "online.num_supervised", 1)
    cfg.online.num_unsupervised = omegaconf_select(cfg, "online.num_unsupervised", 0)
    cfg.online.supervised_param_names = omegaconf_select(cfg, "online.supervised_param_names", ["backbone", "classifier"])
    cfg.online.supervision_source = omegaconf_select(cfg, "online.supervision_source", "NR")
...
def parse_cfg(cfg: omegaconf.DictConfig):
    ...
    # default values for online stuff
    cfg = add_and_assert_online_cfg(cfg)
    ...
    num_gpus = len(cfg.devices) * cfg.num_nodes
    scale_factor = num_gpus
    cfg.optimizer.batch_size = cfg.optimizer.batch_size // scale_factor
    ...
    cfg.resume_wandb_run = omegaconf_select(cfg, "resume_wandb_run", None)
    return cfg
